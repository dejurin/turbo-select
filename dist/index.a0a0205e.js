 ...
 */ const TurboSelect = ({ data, selected, selectLabel = "Select an option", loadingLabel = "Loading...", searchLabel = "Search...", noResults = "No results found.", template = "{label}" })=>{
    ...
    const filteredDataList = (0, _hooks.useMemo)(()=>dataList.filter((item)=>item.label.toLowerCase().includes(searchTerm.toLowerCase()) || item.value.toLowerCase().includes(searchTerm.toLowerCase())), [
        dataList,
        searchTerm
    ]);
    const handleClose = ()=>{
